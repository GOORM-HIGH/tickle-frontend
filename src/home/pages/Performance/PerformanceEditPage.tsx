...
const PerformanceEditPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<PerformanceFormData>({
    title: '', genreId: 0, date: '', runtime: 0, hallType: '',
    hallAddress: '', startDate: '', endDate: '', isEvent: false, img: ''
  });
  ...
  useEffect(() => {
    const fetchPerformance = async () => {
      ...
      try {
        ...
        console.log('서버에서 받은 공연 데이터:', res);
        console.log('event 값:', res.event, '타입:', typeof res.event);
        
        // 백엔드에서 event 필드로 응답
        if ('event' in res) {
          console.log('✅ event 필드가 정상적으로 응답에 포함되었습니다.');
        } else {
          console.warn('⚠️ event 필드가 응답에 없습니다. 기본값을 사용합니다.');
        }
        ...
        setFormData({
          ...
          isEvent: Boolean(res.event), // 백엔드에서 event 필드로 응답
          img: res.img || ''
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('isEvent 전송 값:', formData.isEvent, '타입:', typeof formData.isEvent);
