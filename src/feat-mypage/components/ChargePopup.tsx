...
const ChargePopup: React.FC<ChargePopupProps> = ({
  ...
  const getDisplayAmount = () => {
    const a = selectedAmount ?? Number.parseInt(customAmount, 10);
  ...
  const handleCharge = async () => {
    ...
    try {
      ...
      const paymentData = {
        ...
        items: [
          { id: "tickle_point", name: "Tickle 포인트", qty: 1, price: amount },
        ],
        extra: {
          open_type: "iframe",
          card_quota: [],
      ...
      alert(
        "결제 실패: " +
          (err?.error_message || err?.message || "알 수 없는 오류")
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <div className={styles["amount-grid"]}>
              {[10000, 30000, 50000, 100000, 200000, 500000].map((amount) => (
                <div
                  ...
                  data-selected={amount === (getDisplayAmount() || 0)}
                >
          ...
          <div className={styles["payment-method"]}>
            ...
            <div className={styles["method-grid"]}>
              {["토스"].map((method) => (
                <div
                  key={method}
                  className={styles["method-option"]}
                  onClick={() => handlePaymentMethodSelect(method)}
                  data-selected={method === "토스"}
                >
                  <span>{method}</span>
                </div>
              ))}
